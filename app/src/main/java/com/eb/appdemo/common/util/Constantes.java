package com.eb.appdemo.common.util;

import com.eb.appdemo.db.KeeperDB;

public class Constantes {

    public final static int CANTIDAD_LISTA_SEMANAS=44;
    public final static String DATOS_GENERALES="Datos Generales";
    public final static String CALENDARIO_SEMANA="Semanas Calendario";
    public final static String VALIDAR_SERVICIO="Validar Servicio";
    public final static String DETALLE_NUEVO_MODULO="Resumen del nuevo módulo";


    public static final String[] USER_ERROR_LABEL= {"first name","last name","email",
            "teléfono móvil","país","contraseña"};
    public static final String USER_ERROR_TITLE = "Error";


    //BD
    public static final int VERSION_BD = 1;
    public static final String NOMBRE_BD = "KeeperDB";

}
